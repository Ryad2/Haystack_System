...
int lazily_resize(int resolution, struct imgfs_file *imgfs_file, size_t index) {
    ...
    if (fread(image_buffer, metadata->size[ORIG_RES], NUM_IMGS, imgfs_file->file) !=
        NUM_IMGS) {
        free(image_buffer);
        return ERR_IO;
    }
    ...
    int metadata_file_pointer =
            sizeof(struct imgfs_header) + index * sizeof(struct img_metadata);
    ...
    if (fwrite(metadata, sizeof(struct img_metadata), NUM_IMGS, imgfs_file->file)
            !=  NUM_IMGS) {
        clean_up(in, out, resized_buffer, image_buffer);
        return ERR_IO;
    }
...
#pragma GCC diagnostic ignored "-Wcast-qual"
    const int err = vips_jpegload_buffer((void *) image_buffer, image_size, &original, NULL);

#pragma GCC diagnostic pop
    if (err != ERR_NONE) return ERR_IMGLIB; // Image loading error

    // Retrieve image dimensions
    *height = (uint32_t) vips_image_get_height(original);
    *width = (uint32_t) vips_image_get_width(original);

    // Unreference the VIPS image object
